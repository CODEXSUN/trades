...
This is the authoritative working note for every human or AI agent changing Trades. Read this file first. Open a specialized Assist document only when the task touches that area.
...
- Single-client context or databases: `assist/architecture/tenant-isolation.md`
...
- Trades is a single-client modular monolith with explicit application and leaf-module ownership.
...
- `TRADES_DB_NAME` is the only physical application database. Core's tenant-named context is an internal public-contract compatibility seam and must never become a database or client selector.
...
4. Identify database compatibility and single-client impact.
