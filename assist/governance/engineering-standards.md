...
These standards keep TRADES clean while allowing fast development.
...
Keep module, menu, and route names consistent so application and module registries do not drift.
...
- Application menu routes must follow `/application/<group-key>/<module-key>`.
- Application drill-down child routes must follow `/application/<group-key>/<parent-module-key>/<child-module-key>` and store the child row with `parentModuleId`.
...
- If source inspiration uses camelCase or table-style names, normalize them before adding TRADES registry entries.
