...
- TRADES uses npm only. Run dependency commands from the current Git repository root; pnpm, Yarn, workspace-local installs, and alternative lockfiles are prohibited.
...
- TRADES is a modular monolith unless a stronger reason exists.
...
- The fixed Trades client context is mandatory for business data.
- Events and jobs must include the fixed client context required by their public contracts.
...
- Trades has one configured client and no tenant activation model.
...
- Module access is configured for the one Trades client; tenant-specific plans and entitlement overrides are not product capabilities.
...
- Screens should make client, module, status, and action context clear.
...
- Trades business data belongs to its one configured client database.
...
- Backend validation is required for required fields, duplicate records, status values, relationship references, safe delete blockers, and the fixed client database boundary.
...
- Fast changes must still preserve the fixed single-client database boundary.
