...
TRADES APIs must be tenant-aware, permission-aware, stable, and clear for web, desktop, mobile, workers, integrations, and AI tools.
...
All APIs should return the standard TRADES envelope.
...
| ----------------------------- | ------------------------------------------------------ | ---------------------- |
| Auth (login, session, logout) | `src/platform/api/src/auth/routes.ts`                  | `@trades/platform-api` |
| Application runtime           | `src/platform/api/src/modules/tenant/tenant.routes.ts` | `@trades/platform-api` |
