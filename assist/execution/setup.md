...
If `ENABLE_SINGLE_CLIENT=1`, setup prepares and validates the complete `CLIENT_*` input before changing containers. It provisions the configured tenant and tenant database repeatably; it does not reset an existing tenant database.
