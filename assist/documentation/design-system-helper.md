...
This document is the working standard for every upcoming Trades workspace module. Agents must use it before creating or changing list, show, upsert, lookup, select, date, status, table, and form experiences.
...
- Add `noValidate` to forms so browser-native validation does not replace the Trades banner pattern.
...
- Form tabs must use the shared Trades tone: transparent tab list, one bottom divider under the tab row, animated primary underline, and a small gap before the active tab content.
