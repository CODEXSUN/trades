...
TRADES data design must support tenant isolation, industry customization, offline sync, reporting, accounting correctness, and long-term migration safety.
...
Trades stores Platform infrastructure, its single internal client registry, Core masters/common
data, and client users and permissions in `TRADES_DB_NAME`.

Core Common tables must not add redundant client-selection columns or accept a second database
selector. Core's tenant-named request context remains a signed compatibility seam and must always
resolve to `TRADES_DB_NAME`.
