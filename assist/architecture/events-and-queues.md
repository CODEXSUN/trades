...
TRADES should use events to keep modules independent while allowing them to react to important business moments.
...
TRADES should use Framework queue contracts so queue backends are switchable.
...
- Local/development backend is `database`, controlled by `TRADES_QUEUE_BACKEND=database`.
- `bullmq-redis` is connected through BullMQ and Redis using `TRADES_REDIS_URL`; database metadata is still retained so Queue Management can show status, filters, retries, and audit context.
