...
- For database changes, restore a recent dump locally and run `TRADES_RESTORED_DUMP_TEST=1 npm run db:migrations:test-local`.
- For production database changes, record `TRADES_VERIFIED_BACKUP_ID` before migration execution.
