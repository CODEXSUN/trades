...
TRADES must protect tenant data and recover from failure.
...
- `npm run db:migrations:test-local` runs migrations only after `TRADES_RESTORED_DUMP_TEST=1` confirms a dump has been restored into local databases.
- `npm run db:restore:test` requires `TRADES_RESTORE_TEST_DB_NAME` and verifies the sandbox restore target is reachable.
- `npm run db:backup:verify` requires `TRADES_BACKUP_VERIFY_ID`; only mark a backup verified after a restore test succeeds.
