...
docker exec -e REDISCLI_AUTH="$(env_value TRADES_REDIS_PASSWORD)" codexsun-redis redis-cli ping | grep -qx PONG
...
database_count=$(docker exec -e MYSQL_PWD="$db_password" codexsun-mariadb mariadb --batch --skip-column-names -u "$(env_value DB_USER codexsun_app)" -e "SELECT COUNT(*) FROM information_schema.SCHEMATA WHERE SCHEMA_NAME='$trades_db';")
[ "$database_count" = "1" ] || { echo "Trades application database is missing." >&2; exit 69; }
echo "Trades container smoke test passed: app.trades.in, API, Web, Redis, and the single MariaDB database are ready."
