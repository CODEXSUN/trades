...
async function exerciseTenant(tenant: TenantRow) {
  for (const resource of ["users", "roles", "permissions", "user-roles", "role-permissions"]) {
    const listed = await request(tenant, "GET", `/application/access/${resource}`);
  ...
  for (const [resource, record] of [
    ...
    const shown = await request(tenant, "GET", `/application/access/${resource}/${record.id}`);
    ...
    const off = await request(
      tenant,
      "POST",
      `/application/access/${resource}/${record.id}/deactivate`
    );
    ...
    const on = await request(
      tenant,
      "POST",
      `/application/access/${resource}/${record.id}/activate`
    );
  ...
  for (const [resource, record] of [
    ...
    const removed = await request(
      tenant,
      "DELETE",
      `/application/access/${resource}/${record.id}/force`
    );
...
async function create(tenant: TenantRow, resource: string, payload: unknown) {
  const response = await request(tenant, "POST", `/application/access/${resource}`, payload);
