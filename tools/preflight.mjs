...
const child = spawn(
  ...
  [...config.args, ...(app.endsWith("-web") ? ["--host", host, "--port", String(port)] : [])],
  {
    ...
    env: {
      ...
      ...(app === "platform-api"
        ? {
            TRADES_DB_FRESH_SESSION_FILE: join(
              tmpdir(),
              `trades-platform-fresh-${process.pid}.done`
            )
          }
        : {}),
...
async function freePort(port, host) {
  ...
  if (process.env.TRADES_DEV_PORT_POLICY === "abort") {
    console.error(
      "  x Port policy is abort. Stop the existing process or change TRADES_DEV_PORT_POLICY.\n"
    );
    process.exit(1);
  }
