...
const tradesDb = requiredEnv("TRADES_DB_NAME");
...
GRANT ALL PRIVILEGES ON \`${tradesDb}\`.* TO '${appUser}'@'localhost';
GRANT ALL PRIVILEGES ON \`${tradesDb}\`.* TO '${appUser}'@'127.0.0.1';
FLUSH PRIVILEGES;
`;

const sqlPath = join(root, ".trades-create-db-user.sql");
