...
async function startStack() {
  ...
  await waitForHealthyUrl("http://127.0.0.1:7070/health", "Platform API", 90_000);

  console.log(`  - ${services["platform-web"].label}`);
  startService("platform-web");
  await waitForHealthyUrl("http://127.0.0.1:7080/", "Platform Web", 30_000);
...
function monitorStackHealth() {
  const targets = [
    { failures: 0, label: "Platform API", url: "http://127.0.0.1:7070/health" },
    { failures: 0, label: "Platform Web", url: "http://127.0.0.1:7080/" }
  ];
  let checking = false;
