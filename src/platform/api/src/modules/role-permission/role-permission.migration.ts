...
export async function migrateRolePermissionModule(database: Kysely<TradesDatabase>) {
  await sql
    .raw(
...
async function column(db: Kysely<TradesDatabase>, name: string) {
  const r = await sql<{
    count: number | string;
...
async function addFk(db: Kysely<TradesDatabase>, name: string, col: string, parent: string) {
  const r = await sql<{
    count: number | string;
