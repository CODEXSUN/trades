...
export class DepositService {
  ...
  async list(filters: DepositListFilters = {}) {
    await this.context.authorize("platform.trades.deposit.view");
  ...
  async setStatus(id: string, status: DepositStatus) {
    await this.context.authorize("platform.trades.deposit.lifecycle");
    const current = await this.required(id);
  ...
  async forceDelete(id: string) {
    await this.context.authorize("platform.trades.deposit.delete");
    const current = await this.required(id);
  ...
  private async save<T>(work: () => Promise<T>) {
    try {
      ...
      if (isDuplicate(error)) throw AppError.conflict("Deposit reference already exists.");
      throw error;
    }
  }
