...
export class TenantService {
  ...
  async getTenantRuntime(value: string) {
    ...
    const enabledModuleKeys = runtimeTenant?.enabledModuleKeys ?? [...defaultTenantModuleKeys];
  ...
  async getPublicPortal(value: string): Promise<TenantPublicPortal> {
    ...
    if (!tenant && domain && domain === canonicalDomain && env.CLIENT_CORPORATE_ID) {
      tenant = await this.repository.findByCorporateId(env.CLIENT_CORPORATE_ID);
    }
  ...
  private normalize(input: TenantSavePayload, includeDefaults = false): TenantSavePayload {
    ...
    const disabledModuleKeys = parseStringArray(incomingApps.disabled).filter(
      (key) => !defaultTenantModuleKeys.some((defaultKey) => defaultKey === key)
    );
