...
export class QueueManagerRepository {
  ...
  async settings(): Promise<QueueRuntimeSettings> {
    ...
    return {
      ...
      redisConfigured: Boolean(env.TRADES_REDIS_URL.trim()),
      running: count("running"),
      workerEnabled: env.TRADES_QUEUE_WORKER_ENABLED === "1",
      workerIntervalMs: env.TRADES_QUEUE_WORKER_INTERVAL_MS
    };
  }
  ...
  async configuredBackend() {
    const setting = await getPlatformDatabase()
      ...
      .executeTakeFirst();
    return setting?.backend ?? env.TRADES_QUEUE_BACKEND;
  }
