...
export class CommissionRepository {
  ...
  async list(
    ...
    const lineRows = ids.length
      ? (
          ...
          FROM commission_entry_lines l INNER JOIN commission_variants v ON v.id=l.commission_variant_id
  ...
  async variants() {
    const result = await sql<VariantRow>`
      SELECT id,uuid,code,name,percentage,display_order,status FROM commission_variants
  ...
  async findVariant(id: number) {
    const result =
      await sql<VariantRow>`SELECT id,uuid,code,name,percentage,display_order,status FROM commission_variants WHERE id=${id} LIMIT 1`.execute(
        this.database
      );
  ...
  async updateVariant(id: number, input: CommissionVariantSavePayload) {
    await this.database.transaction().execute(async (transaction) => {
      await sql`UPDATE commission_variants SET name=${input.name},percentage=${input.percentage},status=${input.status} WHERE id=${id}`.execute(
        transaction
      );
      if (input.status === "inactive") {
        await sql`
          DELETE line FROM commission_entry_lines line
        ...
        await sql`
          INSERT INTO commission_entry_lines (uuid,commission_entry_id,commission_variant_id,percentage,amount)
          ...
          FROM commission_entries entry WHERE entry.settled_at IS NULL
  ...
  async settle(direction: CommissionDirection, id: number, actorEmail: string) {
    await sql`UPDATE commission_entries SET settled_at=CURRENT_TIMESTAMP,settled_by=${actorEmail} WHERE id=${id} AND direction=${direction} AND settled_at IS NULL`.execute(
      this.database
    );
  ...
  async findEntry(id: number, direction: CommissionDirection) {
    const result =
      await sql<EntryRow>`SELECT id,uuid,direction,transaction_date,tg_code,name,reference,amount,settled_at,settled_by FROM commission_entries WHERE id=${id} AND direction=${direction} LIMIT 1`.execute(
        this.database
      );
    if (!result.rows[0]) return null;
    const lines = (
      await sql<LineRow>`SELECT l.commission_entry_id,l.commission_variant_id,l.percentage,l.amount,v.name variant_name FROM commission_entry_lines l INNER JOIN commission_variants v ON v.id=l.commission_variant_id WHERE l.commission_entry_id=${id} AND v.status='active' ORDER BY v.display_order,v.id`.execute(
        this.database
      )
    ).rows;
...
export async function syncCommissionSourceEntry(
  ...
  await sql`
    INSERT INTO commission_entries (uuid,direction,source_record_id,transaction_date,tg_code,name,reference,amount)
  ...
  const entry = await sql<{
    id: number;
  }>`SELECT id FROM commission_entries WHERE direction=${input.direction} AND source_record_id=${input.sourceRecordId} LIMIT 1`.execute(
    database
  );
  ...
  const variants = await sql<{
    id: number;
    percentage: number | string;
  }>`SELECT id,percentage FROM commission_variants WHERE status='active'`.execute(database);
...
export async function deleteCommissionSourceEntry(
  ...
  await sql`DELETE FROM commission_entries WHERE direction=${direction} AND source_record_id=${sourceRecordId}`.execute(
    database
  );
}
