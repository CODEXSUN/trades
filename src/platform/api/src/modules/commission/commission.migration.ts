...
export async function migrateCommissionModule(database: Kysely<TenantDatabase>) {
  await sql
    .raw(
      `
    CREATE TABLE IF NOT EXISTS commission_variants (
      ...
      UNIQUE KEY commission_variants_uuid_unique (uuid),
      UNIQUE KEY commission_variants_code_unique (code),
      KEY commission_variants_order_index (display_order)
  ...
  await sql
    ...
    CREATE TABLE IF NOT EXISTS commission_entries (
      ...
      created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
      updated_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
      UNIQUE KEY commission_entries_uuid_unique (uuid),
      UNIQUE KEY commission_entries_source_unique (direction,source_record_id),
      KEY commission_entries_filter_index (direction,settled_at,transaction_date)
    ) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci
  `
    )
    .execute(database);
  await sql
    .raw(
      `
    CREATE TABLE IF NOT EXISTS commission_entry_lines (
      id INT NOT NULL AUTO_INCREMENT PRIMARY KEY,
      uuid CHAR(8) NOT NULL,
      ...
      created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
      updated_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
      UNIQUE KEY commission_entry_lines_uuid_unique (uuid),
      UNIQUE KEY commission_entry_lines_variant_unique (commission_entry_id,commission_variant_id),
      CONSTRAINT commission_entry_lines_entry_fk FOREIGN KEY (commission_entry_id)
        REFERENCES commission_entries(id) ON DELETE CASCADE,
      CONSTRAINT commission_entry_lines_variant_fk FOREIGN KEY (commission_variant_id)
        REFERENCES commission_variants(id)
    ) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci
  `
    )
    .execute(database);
  ...
  for (const variant of defaultVariants) {
    await sql`
      INSERT INTO commission_variants (uuid,code,name,percentage,display_order,status)
  ...
  await backfillSource(database, "deposit", "deposits");
  await backfillSource(database, "withdraw", "payments");
...
async function backfillSource(
  ...
  await sql
    .raw(
      `
    INSERT INTO commission_entry_lines
    ...
    FROM commission_entries entry CROSS JOIN commission_variants variant
