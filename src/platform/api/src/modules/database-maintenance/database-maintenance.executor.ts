...
function liveRestoreName(sourceDatabaseName: string, confirmation: string | undefined) {
  if (
    env.TRADES_ALLOW_LIVE_RESTORE !== "1" ||
    env.TRADES_LIVE_RESTORE_CONFIRM !== "ALLOW_LIVE_RESTORE"
  ) {
    throw new Error(
      "Live restore is disabled. Set TRADES_ALLOW_LIVE_RESTORE=1 and TRADES_LIVE_RESTORE_CONFIRM=ALLOW_LIVE_RESTORE only during an approved restore window."
    );
  }
...
function restoreSandboxName(sourceDatabaseName: string) {
  if (env.NODE_ENV === "production" && !env.TRADES_RESTORE_TEST_DB_NAME) {
    throw new Error("Production restore requires TRADES_RESTORE_TEST_DB_NAME.");
  }
  return assertDatabaseName(
    env.TRADES_RESTORE_TEST_DB_NAME || `${sourceDatabaseName}_restore_sandbox`,
