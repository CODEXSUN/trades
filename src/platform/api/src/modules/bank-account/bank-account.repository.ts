...
export class BankAccountRepository {
  ...
  async list(search = "") {
    ...
    const result = await sql<AccountRow>`
      ...
      LEFT JOIN bank_ledger_entries e ON e.bank_account_id=a.id
  ...
  async update(id: number, input: BankAccountSavePayload) {
    return this.database.transaction().execute(async (transaction) => {
      await sql`
        UPDATE bank_accounts SET code=${input.code},account_name=${input.accountName},bank_name=${input.bankName},
          ifsc=${input.ifsc},branch=${input.branch},opening_balance=${input.openingBalance},status=${input.status}
        WHERE id=${id}
      `.execute(transaction);
      await sql`DELETE FROM bank_ledger_entries WHERE bank_account_id=${id} AND entry_type='opening'`.execute(
        transaction
      );
      if (input.openingBalance !== 0) {
        await insertEntry(transaction, {
  ...
  async setStatus(id: number, status: BankAccountStatus) {
    await sql`UPDATE bank_accounts SET status=${status} WHERE id=${id}`.execute(this.database);
    return this.find(id);
  }
  ...
  async forceDelete(id: number) {
    ...
    await sql`DELETE FROM bank_accounts WHERE id=${id}`.execute(this.database);
    return record;
  }

  async dependencyCount(id: number) {
    const result = await sql<{ count: number | string }>`
      SELECT COUNT(*) count FROM bank_ledger_entries
  ...
  async statement(id: number): Promise<BankStatement | null> {
    ...
    const result = await sql<EntryRow>`
      ...
      LEFT JOIN bank_accounts c ON c.id=e.counterparty_bank_account_id
  ...
  async setReconciled(entryId: number, actorEmail: string, reconciled: boolean) {
    await sql`
      UPDATE bank_ledger_entries SET reconciled_at=${reconciled ? new Date() : null},
        reconciled_by=${reconciled ? actorEmail : null} WHERE id=${entryId}
    `.execute(this.database);
    const result = await sql<{ bank_account_id: number }>`
      SELECT bank_account_id FROM bank_ledger_entries WHERE id=${entryId} LIMIT 1
    `.execute(this.database);
    return result.rows[0] ? this.statement(Number(result.rows[0].bank_account_id)) : null;
  }
}

export async function findActiveBankAccountLink(database: Kysely<TenantDatabase>, id: number) {
  const result = await sql<{
    account_name: string;
    bank_name: string;
    code: string;
    id: number;
    status: BankAccountStatus;
  }>`SELECT id,code,account_name,bank_name,status FROM bank_accounts WHERE id=${id} LIMIT 1`.execute(
    database
  );
...
export async function deleteBankLedgerSourceEntry(
  ...
  await sql`DELETE FROM bank_ledger_entries WHERE source_module=${sourceModule} AND source_record_id=${sourceRecordId}`.execute(
    database
  );
}
