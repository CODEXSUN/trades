...
export class BankAccountService {
  ...
  async list(search = "") {
    await this.context.authorize("platform.trades.bank-account.view");
  ...
  async create(input: BankAccountSavePayload) {
    await this.context.authorize("platform.trades.bank-account.create");
  ...
  async update(id: string, input: BankAccountSavePayload) {
    await this.context.authorize("platform.trades.bank-account.update");
  ...
  async setStatus(id: string, status: BankAccountStatus) {
    await this.context.authorize("platform.trades.bank-account.lifecycle");
    const current = await this.required(id);
  ...
  async forceDelete(id: string) {
    await this.context.authorize("platform.trades.bank-account.delete");
    const current = await this.required(id);
  ...
  async createManualEntry(id: string, input: BankManualEntryPayload) {
    await this.context.authorize("platform.trades.bank-account.entry");
  ...
  async transfer(input: BankTransferPayload) {
    await this.context.authorize("platform.trades.bank-account.transfer");
  ...
  async setReconciled(entryId: string, reconciled: boolean) {
    await this.context.authorize("platform.trades.bank-account.reconcile");
