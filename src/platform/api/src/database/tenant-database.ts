...
export async function createTenantDatabase(target: string | Tenant) {
  ...
  try {
    ...
    console.info(`[database] internal client database context ready: "${name}"`);
  } finally {
...
export function getTenantDatabase(tenant: Tenant) {
  ...
  const database = new Kysely<TenantDatabase>({
    dialect: new MysqlDialect({
      pool: createPool({
        database: assertTradesDatabaseTarget(tenant.dbName, "internal client database name"),
        connectionLimit: 10,
...
export function getTenantDatabaseByName(databaseName: string) {
  const name = assertTradesDatabaseTarget(databaseName, "internal client database name");
...
export async function dropTenantDatabase(tenant: Tenant) {
  ...
  const databaseName = assertTradesDatabaseTarget(tenant.dbName, "internal client database name");
  console.warn(
    `[database] dropping Trades database "${databaseName}" for internal client "${tenant.tenantCode}"`
  );
  const connection = await createConnection({
    host: tenant.dbHost || env.DB_HOST,
    password: env.DB_PASSWORD,
...
function assertTradesDatabaseTarget(value: string, label: string) {
  const name = assertDatabaseName(value, label);
  const configuredName = assertDatabaseName(env.TRADES_DB_NAME, "Trades database name");
  if (name !== configuredName) {
    throw new Error(`${label} must be the configured Trades database "${configuredName}".`);
  }
  return name;
}
