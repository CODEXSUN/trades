...
const TOKEN_KEYS: Record<Desk, string> = {
  admin: "trades_session_admin",
  sa: "trades_session_sa",
  tenant: "trades_session_client"
};

const TENANT_ID_KEY = "trades_client_id";
const TENANT_DB_NAME_KEY = "trades_client_db_name";
...
const TENANT_RUNTIME_KEYS = [
  "trades.client.landing-app.live",
...
function writeTenantSession(input: {
  ...
  // storage contract. Mirror the authenticated Trades context at that seam.
  try {
...
export async function login(input: { email: string; password: string }) {
  ...
  try {
    ...
    if (!data.accessToken || !data.tenantId || !data.tenantDbName) {
      throw new Error("Client login response is incomplete.");
    }
    writeTenantSession({
...
export async function developmentClientLogin() {
  clearTenantSession();
